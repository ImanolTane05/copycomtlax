...
const Encuestas = () => {
    ...
    const handleChange = (encuestaId, preguntaId, value, tipo) => {
        setRespuestas((prev) => {
            const respuestasEncuesta = prev[encuestaId] || {};
            const valorActual = respuestasEncuesta[preguntaId];

            let nuevoValor;

            if (tipo === 'Opción múltiple') {
                // Lógica para Arrays (Checkboxes)
                const arregloActual = Array.isArray(valorActual) ? valorActual : [];
                
                if (arregloActual.includes(value)) {
                    // Si ya existe, lo sacamos (desmarcar)
                    nuevoValor = arregloActual.filter((item) => item !== value);
                } else {
                    // Si no existe, lo agregamos (marcar)
                    nuevoValor = [...arregloActual, value];
                }
            } else {
                // Lógica normal para Strings (Radio buttons / Texto)
                nuevoValor = value;
            }
    ...
    const handleSubmit = async (e, encuestaId) => {
        ...
        // Ajusta la validación para soportar arreglos vacíos o strings vacíos
        const preguntasSinRespuesta = encuesta.preguntas.filter((p) => {
            const respuesta = respuestasEncuesta[p._id];
            
            // Si es null o undefined
            if (!respuesta) return true;
            // Si es array vacío (caso multiple)
            if (Array.isArray(respuesta) && respuesta.length === 0) return true;
            // Si es string vacío (caso texto/radio)
                                    ...
                                    <div key={pregunta._id} className="mb-6 bg-white p-4 rounded-xl shadow-md">
                                        ...
                                        {pregunta.tipo === 'Cerrada' || pregunta.tipo === 'Opción múltiple' ? (
                                            pregunta.opciones.map((opcion, idx) => (
                                                ...
                                                >
                                                    <input
                                                        ...
                                                        // CORRECCIÓN VISUAL (CHECKED):
                                                        // Verifica si es array (multiple) o string (cerrada)
                                                        checked={
                                                            pregunta.tipo === 'Opción múltiple'
                                                                ? (respuestas[encuesta._id]?.[pregunta._id] || []).includes(opcion)
                                                                : respuestas[encuesta._id]?.[pregunta._id] === opcion
                                                        }
                                                        
                                                        // PASAMOS EL TIPO DE PREGUNTA:
                                                        onChange={(e) =>
                                            ...
                                            <textarea
                                                ...
                                                // Pasamos el tipo 'Abierta' explícitamente o dejamos que caiga en el 'else'
                                                onChange={(e) =>
                                                    handleChange(encuesta._id, pregunta._id, e.target.value, 'Abierta')
                                                }
